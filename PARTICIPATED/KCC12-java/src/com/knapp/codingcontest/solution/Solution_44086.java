...
public class Solution_44086 {
  ...
  public Solution_44086(final Warehouse warehouse, final InputData input) {
  ...
  public void run() throws Exception {
    ...
    while (!allOrders.isEmpty()) {
      ...
      Set<Product> temptemp = new HashSet<>(workStation.getAssignedProducts());
      temptemp.removeAll(allOrders.get(0).getAllProducts());
      for (Product p : temptemp) {
    ...
    workStation.assignProduct(current.getOpenProducts().get(0));
    workStation.pickOrder(current, current.getOpenProducts().get(0));
  }
