 ...
 */
public class Solution_CLUSTERFUCK2 {
  ...
  public Solution_CLUSTERFUCK2(final Warehouse warehouse, final InputData input) {
  ...
  public void run() throws Exception {
    ...
    allOrders = new ArrayList<>(allOrders.stream().filter(i -> i.getAllProducts().size() >= 4).toList());

    while (!allOrders.isEmpty()) {
      Order current = allOrders.removeFirst();
      System.out.println("ACTIVE:" + workStation.getActiveOrders());
      ...
      while (!ordered.isEmpty()) {
        for (int i = 0; i < 3; i++) {
          try {
            workStation.assignProduct(ordered.removeFirst());
          } catch (Exception e) {
            break;
          }
        }

        List<Order> exact = EXACToverlapWith4Prods(workStation.getAssignedProducts(), allOrders);
        //System.out.println(exact);

        for (Order o : exact) {
          if (workStation.getActiveOrders().size() == 4) break;
          workStation.startOrder(o);
          System.out.println(o.getAllProducts());
