...
public class Scheduler {
   ...
   * @throws Exception
   */
  public Scheduler(final ProductCollection productCollection, final PickOrderCollection pickOrderCollection,
      final LocationCollection locationCollection) throws Exception {
   ...
   * @throws Exception
   */
  public List<Map.Entry<Integer, ReplenishmentOrder>> run(final Solution solution) throws Exception {
    Contract.requires(null != solution, "solution required but is null");

    System.err.println(String.format("### Starting work..."));
    final List<Map.Entry<Integer, ReplenishmentOrder>> replenOrders = new ArrayList<Map.Entry<Integer, ReplenishmentOrder>>();
    ...
    while ((pickOrderCollection.count() > 0) && (currentTick < Settings.MAX_ALLOWED_CYCLES)) {
      ...
      try {
        if (replenOrder != null) {
          replenOrders.add(new AbstractMap.SimpleEntry<Integer, ReplenishmentOrder>(Integer.valueOf(currentTick), replenOrder));
    ...
    if (pickOrderCollection.count() > 0) {
      System.err.println(String.format("### Remaining work: %d orders with %d lines", pickOrderCollection.count(),
  ...
  private boolean refillLocations(final ReplenishmentOrder replenishmentOrder, final int currentTick) {
    ...
    if (target == null) {
      System.err.println(String.format("ERROR @ %d: target location '%s' not found for order %s: ", currentTick,
          replenishmentOrder.getReplenishedLocation(), replenishmentOrder.getOrderId()));
      return false;
    }
    ...
    if ((target.getAssignedProduct() != null) && !target.getAssignedProduct().getCode().equals(product.getCode())
  ...
  private List<PickOrder> tryPickOrders(final int currentTick) {
    ...
    System.err.println(String.format("[Cyle %d] Picked %d orders, orders left: %d", currentTick, pickedOrders.size(),
